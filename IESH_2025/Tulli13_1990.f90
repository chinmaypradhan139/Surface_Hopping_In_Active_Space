...
implicit none
  ...
  integer :: ham_sp,hp,ntraj,locate
  integer :: n_q,iter,traj_no
  integer :: hop_flg,collapse_flg 
  real*8, dimension(:), allocatable :: inpot,transE
  integer, dimension(:),allocatable :: finalst,outputs
  integer , dimension(:,:), allocatable :: reachable
  real*8, dimension(:,:), allocatable :: tdata
  integer :: outdims,out_freq,pop_avg_time,pop_cal_methods
  real*8 :: barrier,gamma_B
  integer, dimension(:), allocatable :: bins
  real*8, dimension(:,:,:), allocatable :: out_dist,avg_dist
  real*8 :: above_barrier
contains
!.......................................................................
...
pop_cal_methods=2
  ...
  allocate(e_metal(Hi))
  allocate(transE(Ne+1))
  allocate(reachable(Ne+1,Ne))
  allocate(finalst(Hi))
  allocate(out_dist(out_freq,Hi,pop_cal_methods))
  allocate(avg_dist(out_freq,Hi,pop_cal_methods))
...
diabstate=numstate(binstate(diabstate),1) !sorts in assending order
...
above_barrier=0.d0
pstate=binstate(diabstate)
do i=2,Hi
    if (H(i,i)>barrier) then
...
!do i=1,Ne**2+1
!    plus2state(:,:,i)=excite1(Hi_sp(i,:))
!enddo

!write(67,*) time, lambda
!do i=1,Ne**2+1
!    do k=1,Ne**2+1
!        write(67,*)k,'k',i,'i', plus2state(k,:,i) 
!    enddo
!enddo
!call construct_state_space(Hi_sp)
...
!do j=1,Ne**2+1
!    do i=1,Ne**2+1
!        call calculate_sigma(plus2state(i,:,j),Aii,1)
!        tr_A=tr_A+Aii
!    enddo
!enddo
!call cpu_time(finish)

!write(26,*) time,finish-start
...
!if (mod(int(time),5000).eq.0) then
...
!   write(45,*)time,i,real(sum(c(i,1:Hi)*conjg(c(i,1:Hi))))
!   if ((real(modcsq)<0.99).or.(real(modcsq)>1.01)) then
!       write(47,*) time,i,modcsq
!   end if
!enddo
!..................................
...
!................................................................................
function excite2(istate)
implicit none
!integer excite2(Ne,Ne**2),
integer :: istate(Ne)
integer :: i,j,n
!integer, dimension(:,:), allocatable :: excite2 
integer :: excite2

n=1
do i=1,Ne
    do j=1,Hi
        if (.not.(any(j==lambda.and.j==istate))) then
!            excite2(i)=j
            n=n+1
        end if
    enddo
...
!       nex=nex+rho_d(i,i,k) 
!    enddo
!    pchem=pchem+nex*gfac(Energy(i),Energy(sH))
...
!....................................................................................
function excite1(istate)
implicit none
integer :: istate(Ne),excite1(Ne**2+1,Ne)
...
subroutine check_collapse
  ...
  real*8 :: clamb,cmod,N_ele1,N_ele2,f_diff(Ne**2,total_dimensions)
...
enddo
 ...
 do n=2,Ne**2+1
   ...
   call random_number(rnd)
   f_diff(n-1,:)= new_force(n,:)-new_force(1,:)
...
!write(56,*)time, maxval(f_diff), gama_reset
...
do j=1,Hi
    syspop(j)=sum(rho_d(j,j,1:Ne))
enddo
!write(411,*) time,sum(syspop(1:Hi))
...
!write(301,*) time,cos(thetac),exp(-Dx/(lamb*cos(thetac)))

do i=0,1000
    theta=i*dth
    gfac=gfac+dth*sin(theta)*exp(-Dx/(lamb*cos(theta)))
    !if (exp(-Dx/(lamb*cos(theta)))>100) then
    
   ! write(302,*) i,thetac,theta
   ! write(302,*) exp(-Dx/(lamb*cos(theta))),cos(theta),theta
   ! write(302,*) exp(-Dx/(lamb*cos(thetac))),cos(thetac),thetac
...
iloop : do i=1,Ne
    ...
    exo=diab_energy(numstate(outst,1))-diab_energy(numstate(temp,1))
