...
real*8 s01,s02,x_cr
real*8,allocatable :: mass(:),omg(:),ck(:),dk(:),alpha(:)
...
real*8,allocatable :: Hamil_site(:,:),Hamil_site1(:,:),Hamil_diab(:,:),delH_dels(:,:,:),delH_dels_ad(:,:,:)
...
integer :: mol,init_state
integer, allocatable:: pot_in(:)
contains
!---------------------------------------------------------- 
!---------------------------------------------------------- 
...
!stop
  ...
  allocate(mass(nclass),omg(nclass),ck(nclass),dk(nclass),alpha(nclass))
  ...
  allocate(Hamil_diab_0(nbasis,nbasis),pot_in(nquant))
...
!    call execute_command_line("python3 manual.py",stat)
    write(112,*) stat
    call init_cond
    !call evolve(1,10000)

    !write(174,*) curr_time/au2s,x(1)/au2m
    call setup_parameters
...
!  do i=1,nclass-1
!  x(i)=x(i)+ck(i)*x(nclass)/(mass(i)*omg(i)**2)
!  enddo

! x(nclass)=x(nclass)-g_coup/(mass(1)*omg_B**2)
...
subroutine evolve(start,nsteps)
  ...
  do i=start,nsteps
    ...
    write(174,*) x(1)/au2m,V_k(state)/au2J,curr_time/au2s!vdotd(1,2)*au2m
...
subroutine write_average
  ...
  implicit none
  integer i,j,i1,k
  real*8 nf,pop_el(2),p1,p2,pops,r1,r2,r3
  ...
  do i=1,nsteps/nstep_avg
    ...
    if (mol==2) then
        ...
        p2=0.d0
        do j=1,nquant
            if (pot_in(j)==1.or.pot_in(j)==3) p1=p1+real(rho(j,j,i))
            if (pot_in(j)==2.or.pot_in(j)==3) p2=p2+real(rho(j,j,i))
        enddo
        write(100,'(21f15.7)')(i-1)*nstep_avg*dtc/au2s,p1,p2
    else
        pops=0.d0
        do j=1,nquant
            if (pot_in(j)==1) pops=pops+real(rho(j,j,i))
        enddo
        write(100,'(21f15.7)')(i-1)*nstep_avg*dtc/au2s,pops
    end if
    ...
    r3=0.d0
    do j=1,nquant
        if (pot_in(j)==1) r1=r1+real(rho(j,j,i))
        if (pot_in(j)==2) r2=r2+real(rho(j,j,i))
        if (pot_in(j)==3) r3=r3+real(rho(j,j,i))
    enddo
    !write(100,'(21f15.7)')(i-1)*nstep_avg*dtc/au2s,r1,r2,r3

!    write(100,'(71f15.7)') (i-1)*nstep_avg*dtc/au2s,(real(rho(j,j,i)), j=1,nquant)
        


    !think
!   write(100,'(21f15.7)')(i-1)*nstep_avg*dtc/au2s,1d0-real(rho(3,3,i))
!   write(100,'(21f15.7)')(i-1)*nstep_avg*dtc/au2s,real(rho(1,1,i)+rho(2,2,i)+rho(3,3,i)),real(rho(1,1,i)+rho(4,4,i)+rho(5,5,i))
  enddo
...
Hamil_site=0.d0
Hamil_site(3,3)=0.d0
Hamil_site(1,1)=-0.003d0-0.002/2
Hamil_site(2,2)=-0.003d0+0.002/2 
!
Hamil_site(2,3)=2.82d-4; Hamil_site(3,2)=Hamil_site(2,3)
Hamil_site(1,3)=2.82d-4; Hamil_site(3,1)=Hamil_site(1,3)

Hamil_site=Hamil_site*au2J


write(17,*) 'bm'
  do i=1,nquant
    do j=1,nquant
    write(17,'(es10.2$)') Hamil_site(i,j)/au2J!/wave_to_J
    enddo
  write(17,*) 
  enddo
open(28,file='pot_in.txt')
   do i=1,nquant
      read(28,*) pot_in(i)
   enddo
close(28)
...
subroutine compute_potential3(H_diab,delV_dels)
implicit none
...
do i =1,nquant
    if (i==3) then
        H_diab(i,i)=H_diab(i,i)+0.5*mass(1)*omg_B**2*x(1)**2
        delv_dels(i,i,1)=delv_dels(i,i,1)+mass(1)*omg_B**2*x(1)
    else
        H_diab(i,i)=H_diab(i,i)+0.5*mass(1)*omg_B**2*(x(1)-g_coup)**2
        delv_dels(i,i,1)=delv_dels(i,i,1)+mass(1)*omg_B**2*(x(1)-g_coup)
    end if
enddo
...
!  do i=1,nquant
!    do j=1,nquant
!    write(18,'(es10.2$)') H_diab(i,j)/au2J!/wave_to_J
!    enddo
!  write(18,*) 
!  enddo

end subroutine compute_potential3
...
call read_real_matrix("Hamil_site.txt",nquant,nquant,Hamil_site)
...
mass=input(1)*au2kg
lambda_B=input(2)*au2J
omg_B=input(3)/au2s
V_exothermicity=input(4)*au2J
omg=omg_B
gamma_B=2*omg_B
...
subroutine read_real_matrix(filename, rows, cols, matrix)
    implicit none
    character(len=*), intent(in) :: filename
    integer, intent(in) :: rows, cols
    real*8, intent(out) :: matrix(rows,cols)
    integer :: i, j, status
...
integer i,j,k1,k
  ...
  g_coup=sqrt(2.0*lambda_B/(mass(1)*omg_B**2))
!
!write(43,*)lambda_B/au2J,V_exothermicity/au2J,omg_B*au2s,mass/au2kg,g_coup/au2m,temperature
  H_diab=Hamil_site
  delv_dels=0.d0
...
if (mol==2) then
  do i=1,nquant
      if (pot_in(i)==3) then
            H_diab(i,i)=H_diab(i,i)+0.5*mass(1)*omg_B**2*(x(1)-2*g_coup)**2+2*V_exothermicity
            delv_dels(i,i,1)=delv_dels(i,i,1)+mass(1)*omg_B**2*(x(1)-2*g_coup)
      else if (pot_in(i)==1 .or. pot_in(i)==2) then
          H_diab(i,i)=H_diab(i,i)+0.5*mass(1)*omg_B**2*(x(1)**2-2*g_coup*x(1)+2*g_coup**2)+V_exothermicity
          delv_dels(i,i,1)=delv_dels(i,i,1)+mass(1)*omg_B**2*(x(1)-g_coup)
      else
            H_diab(i,i)=H_diab(i,i)+0.5*mass(1)*omg_B**2*x(1)**2
            delv_dels(i,i,1)=delv_dels(i,i,1)+mass(1)*omg_B**2*x(1)
...
if (mol==1) then
    do i=1,nquant
        if (pot_in(i)==1) H_diab(i,i)=H_diab(i,i)+0.5*mass(1)*omg_B**2*(x(1)-g_coup)**2+V_exothermicity
        if (pot_in(i)==1) delv_dels(i,i,1)=delv_dels(i,i,1)+mass(1)*omg_B**2*(x(1)-g_coup)
        if (pot_in(i)==0) H_diab(i,i)=H_diab(i,i)+0.5*mass(1)*omg_B**2*x(1)**2
        if (pot_in(i)==0) delv_dels(i,i,1)=delv_dels(i,i,1)+mass(1)*omg_B**2*x(1)
    enddo
end if
...
subroutine setup_parameters2
  implicit none
  integer i
  real*8 si_diab(nbasis,2),Vb,U_coup,e3_met,e4_met,Vij


  mass=2000.d0*au2kg

  omg_B=2.d-4/au2s
  lambda_B=0.005d0*au2J
  V_exothermicity=-0.003d0*au2J
  U_coup=0.007d0*au2J
  e3_met=-2.d-3*au2J
  e4_met=2.d-3*au2J
  Vij=1.d-4*au2J
  ...
  Hamil_site=0.d0
  Hamil_site(1,1)=U_coup
  Hamil_site(2,2)=e3_met
  Hamil_site(3,3)=e4_met
  Hamil_site(4,4)=e3_met
  Hamil_site(5,5)=e4_met
  Hamil_site(6,6)=e3_met+e4_met

  Hamil_site(1,2)=Vij ; Hamil_site(2,1)=Hamil_site(1,2)
  Hamil_site(1,3)=Vij ; Hamil_site(3,1)=Hamil_site(1,3)
  Hamil_site(1,4)=Vij ; Hamil_site(4,1)=Hamil_site(1,4)
  Hamil_site(1,5)=Vij ; Hamil_site(5,1)=Hamil_site(1,5)
  Hamil_site(1,6)=0.d0; Hamil_site(6,1)=Hamil_site(1,6)

  Hamil_site(2,3)=0.d0 ; Hamil_site(3,2)=Hamil_site(2,3)
  Hamil_site(2,4)=0.d0 ; Hamil_site(4,2)=Hamil_site(2,4)
  Hamil_site(2,5)=0.d0 ; Hamil_site(5,2)=Hamil_site(2,5)
  Hamil_site(2,6)=Vij  ; Hamil_site(6,2)=Hamil_site(2,6)

  Hamil_site(3,4)=0.d0 ; Hamil_site(4,3)=Hamil_site(3,4)
  Hamil_site(3,5)=0.d0 ; Hamil_site(5,3)=Hamil_site(3,5)
  Hamil_site(3,6)=Vij  ; Hamil_site(6,3)=Hamil_site(3,6)

  Hamil_site(4,5)=0.d0 ; Hamil_site(5,4)=Hamil_site(4,5)
  Hamil_site(4,6)=Vij  ; Hamil_site(6,4)=Hamil_site(4,6)

  Hamil_site(5,6)=Vij  ; Hamil_site(6,5)=Hamil_site(5,6)
...
subroutine compute_potential2(H_diab,delV_dels)
  implicit none
  real*8,intent(out) :: H_diab(nquant,nquant),delV_dels(nquant,nquant,nclass)
  integer i,j,k1
  real*8 h1,dh1(nclass),diff,d_diff(nclass)

!write(43,*)lambda_B/au2J,V_exothermicity/au2J,omg_B*au2s,mass/au2kg,g_coup/au2m,temperature
  H_diab=Hamil_site
  delv_dels=0.d0

  H1=0.25*mass(1)*omg_B**2*x(1)**2
  dH1=0.5*mass*omg_B**2*x

  diff = 2*lambda_B+V_exothermicity - mass(1)*omg_B**2*g_coup*x(1)
  d_diff = - mass(1)*omg_B**2*g_coup

  do i=1,nquant
    H_diab(i,i)=H_diaB(i,i)+2*H1
    delv_dels(i,i,:)=2*dH1
  enddo

  H_diab(1,1)=H_diab(1,1)+2*diff
  delv_dels(1,1,:)=delv_dels(1,1,:)+2*d_diff

  do i=2,5
    H_diab(i,i)=H_diab(i,i)+diff
    delv_dels(i,i,:)=delv_dels(i,i,:)+d_diff
  enddo

end subroutine compute_potential2
...
state=1
  ...
  do i=1,100
    !x(1)=-20.d-10+50.d-10*i/100.d0
    x(1)=-20.d0+80.d0*i/100.d0
    x=x*au2m
    call evaluate_variables(0)
    write(20,*) x/au2m,V_k/au2J
    do j=1,nquant
        diabs(j)=Hamil_diab(j,j)
    enddo
    write(21,*) x/au2m, diabs/au2J
  enddo
  stop
...
spectral=lambda_B*gamma_B*w/(gamma_B**2+w*w)
